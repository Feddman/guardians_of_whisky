...
export default function WhiskyReveal({ whisky }: WhiskyRevealProps) {
  ...
  const findMatches = () => {
    const matches: { category: string; value: string; count: number }[] = []
    ...
    Object.entries(colorCounts).forEach(([color, count]) => {
      if (count > 1) {
        matches.push({ category: 'Color', value: color, count })
      }
    })
    ...
    Object.entries(overallCounts).forEach(([rating, count]) => {
      if (count > 1) {
        matches.push({ category: 'Overall Rating', value: `${rating} stars`, count })
      }
    })
    ...
    Object.entries(noteCounts).forEach(([note, count]) => {
      if (count > 1) {
        matches.push({ category: 'Flavor Note', value: note, count })
      }
    })
            ...
            {matches.map((match, idx) => (
              <div key={idx} className="text-sm">
                ...
                <span className="text-whisky-dark ml-2">
                  ({match.count} {match.count === 2 ? 'personen' : 'personen'} matchten)
                </span>
              </div>
            ))}
          </div>
          ...
          >
            ...
            {rating.nose && (
              <div className="mt-2 text-sm">
                <span className="font-medium">Neus:</span> {rating.nose}
              </div>
            )}
            {rating.palate && (
              <div className="mt-2 text-sm">
                <span className="font-medium">Smaak:</span> {rating.palate}
              </div>
            )}
            {rating.finish && (
              <div className="mt-2 text-sm">
                <span className="font-medium">Afsmaak:</span> {rating.finish}
              </div>
            )}
